...
export default function TradingPage() {
  ...
  const bestPrice = Math.min(...filteredExchanges.map((e) => e.price));
  const worstPrice = Math.max(...filteredExchanges.map((e) => e.price));
  const avgPrice = filteredExchanges.reduce((sum, e) => sum + e.price, 0) / filteredExchanges.length;
        ...
        <div className="border-b border-slate-700 bg-slate-900/50 backdrop-blur-sm sticky top-16 z-40">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {[
                { id: 'ranking' as ViewMode, label: '📊 Smart Ranking' },
                { id: 'heatmap' as ViewMode, label: '🔥 Heatmap' },
                { id: 'comparison' as ViewMode, label: '⚙️ Comparison' },
                { id: 'sparklines' as ViewMode, label: '📈 Sparklines' },
                { id: 'insights' as ViewMode, label: '💡 Insights' },
                { id: 'network' as ViewMode, label: '🌐 Network' },
              ].map((mode) => (
                <button
                  key={mode.id}
            ...
            <div className="lg:col-span-1">
              <div className="bg-slate-800 rounded-lg p-6 border border-slate-700 sticky top-32">
                ...
                <div className={`space-y-4 ${showFilters ? 'block' : 'hidden lg:block'}`}>
                  ...
                  {/* Quality Filter */}
                  <div>
                    <label className="text-xs text-slate-400 block mb-2">Exchange Quality</label>
                    <select
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[5, 10, 15, 20].map((pct) => (
                        <button
                          key={pct}
                          onClick={() => setFilters({ ...filters, priceRange: pct })}
            ...
            <div className="lg:col-span-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
                  ...
                  <p className="text-2xl font-bold text-green-400 mt-2">${bestPrice.toFixed(2)}</p>
                </div>
                <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
                  <p className="text-slate-400 text-sm">Average Price</p>
                  <p className="text-2xl font-bold text-blue-400 mt-2">${avgPrice.toFixed(2)}</p>
                </div>
                <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
                  <p className="text-slate-400 text-sm">Worst Price</p>
                  <p className="text-2xl font-bold text-orange-400 mt-2">${worstPrice.toFixed(2)}</p>
                </div>
                <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
                  <p className="text-slate-400 text-sm">Spread</p>
                  <p className="text-2xl font-bold text-yellow-400 mt-2">
                    {((worstPrice - bestPrice) / bestPrice * 100).toFixed(2)}%
                  </p>
                </div>
              </div>

              {/* View Content */}
              {viewMode === 'ranking' && <SmartRankingView exchanges={filteredExchanges} bestPrice={bestPrice} />}
              {viewMode === 'heatmap' && <HeatmapView exchanges={filteredExchanges} bestPrice={bestPrice} />}
              {viewMode === 'comparison' && <ComparisonView exchanges={filteredExchanges} />}
              {viewMode === 'sparklines' && <SparklinesView exchanges={filteredExchanges} />}
              {viewMode === 'insights' && <InsightsView exchanges={filteredExchanges} avgPrice={avgPrice} />}
            </div>
          </div>
        </div>
      </div>
...
const SmartRankingView = ({ exchanges, bestPrice }: ViewComponentProps) => (
...
const HeatmapView = ({ exchanges, bestPrice = 0 }: ViewComponentProps) => (
  <div className="space-y-2">
    {exchanges.map((ex: ExchangeData) => {
      const pricePercent = (ex.price - bestPrice) / bestPrice * 100;
      ...
      return (
        <div key={ex.name} className="flex items-center gap-4">
          ...
          <div className={`flex-1 h-10 rounded transition-colors ${color}`} style={{opacity: 0.3 + (colorIntensity / 200)}} />
...
const ComparisonView = ({ exchanges }: ViewComponentProps) => (
...
const SparklinesView = ({ exchanges }: ViewComponentProps) => (
...
const InsightsView = ({ exchanges, avgPrice = 0 }: ViewComponentProps) => (
