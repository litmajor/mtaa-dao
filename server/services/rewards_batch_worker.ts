  ...
  (async function loop() {
    while (running) {
      try {
        ...
        console.warn('rewards_batch_worker loop error:', err instanceof Error ? err.message : err);
      }
...
async function processPendingBatch() {
  ...
  const pending = await db.select().from(reward_requests).where(sql`${reward_requests.status} = 'pending' AND ${reward_requests.attempts} < ${MAX_ATTEMPTS}`).limit(BATCH_SIZE);
  ...
  try {
    ...
    console.warn('rewards_batch_worker processing failed:', err.message || err);
