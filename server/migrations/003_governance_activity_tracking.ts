...
export async function up(db: PostgresDB) {
  ...
  await db.execute(sql`
    ...
    CREATE INDEX IF NOT EXISTS idx_activity_log_user_dao ON governance_activity_log(user_id, dao_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_activity_log_dao_type ON governance_activity_log(dao_id, type, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_activity_log_expires ON governance_activity_log(expires_at) WHERE expires_at IS NOT NULL;
    CREATE INDEX IF NOT EXISTS idx_activity_log_points ON governance_activity_log(user_id, dao_id, points DESC);
  `);
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS governance_promotion_history (
      ...
      promoted_by VARCHAR(50) NOT NULL DEFAULT 'system',
      created_at TIMESTAMP NOT NULL DEFAULT NOW()
    );

    CREATE INDEX IF NOT EXISTS idx_promotion_history_user_dao ON governance_promotion_history(user_id, dao_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_promotion_history_dao ON governance_promotion_history(dao_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_promotion_history_promoted_by ON governance_promotion_history(promoted_by);
  `);
  ...
  await db.execute(sql`
    ...
    CREATE INDEX IF NOT EXISTS idx_promotion_requests_user_dao ON governance_promotion_requests(user_id, dao_id, status);
    CREATE INDEX IF NOT EXISTS idx_promotion_requests_dao_status ON governance_promotion_requests(dao_id, status, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_promotion_requests_pending ON governance_promotion_requests(status) WHERE status = 'pending';
  `);
  ...
  // Add missing columns to dao_members if they don't exist
  await db.execute(sql`
  ...
  // Create trigger function to update activity_points
  await db.execute(sql`
  ...
  // Create trigger
  await db.execute(sql`
    DROP TRIGGER IF EXISTS trg_update_activity_points ON governance_activity_log;
  ...
  console.log('✅ Governance activity tracking tables created');
}

export async function down(db: PostgresDB) {
  // Drop in reverse order of creation
  await db.execute(sql`DROP TRIGGER IF EXISTS trg_update_activity_points ON governance_activity_log;`);
  await db.execute(sql`DROP FUNCTION IF EXISTS update_user_activity_points();`);
  await db.execute(sql`DROP VIEW IF EXISTS vw_promotion_eligibility;`);
  await db.execute(sql`DROP VIEW IF EXISTS vw_user_activity_stats;`);
  await db.execute(sql`DROP TABLE IF EXISTS governance_role_requirements;`);
  await db.execute(sql`DROP TABLE IF EXISTS governance_activity_types;`);
  await db.execute(sql`DROP TABLE IF EXISTS governance_promotion_requests;`);
  await db.execute(sql`DROP TABLE IF EXISTS governance_promotion_history;`);
  await db.execute(sql`DROP TABLE IF EXISTS governance_activity_log;`);

  console.log('✅ Governance activity tracking tables dropped');
}
