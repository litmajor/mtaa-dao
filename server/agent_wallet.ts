...
export class EnhancedAgentWallet {
  ...
  async getBalanceEth(address?: string): Promise<number> {
    const targetAddress = address || this.account.address;
    const balance = await this.web3.eth.getBalance(targetAddress);
    return parseFloat(this.web3.utils.fromWei(balance, 'ether'));
  }
  ...
  async getTokenInfo(tokenAddress: string, includePrice: boolean = false): Promise<TokenInfo> {
    try {
  ...
  async getEnhancedPortfolio(tokenAddresses: string[]): Promise<Portfolio> {
  ...
  async estimateDisbursementCost(disbursements: Disbursement[]): Promise<{
    totalGasCost: number;
...
export class WalletManager {
  ...
  static validatePrivateKey(privateKey: string): boolean {
    try {
      ...
      new Web3().eth.accounts.privateKeyToAccount(privateKey);
      return true;
    } catch {
      return false;
    }
  ...
  static isContract(web3: Web3, address: string): Promise<boolean> {
...
export class DaoTreasuryManager {
  ...
  async getTreasurySnapshot(): Promise<{
    ...
    for (const tokenAddress of this.allowedTokens) {
      try {
